...
def add_tracks(playlist_id):
	...
	db = get_db()
	try:
		sqlQry1 = "select track_id from tracks where track_title='%s'" %(track_title)
		...
		if len(res)>0:
			...
			track_id = res[0][0]
			sqlQry2 = "insert into playlist_tracks(playlist_id,track_id) values ('%s','%s')" %(playlist_id,track_id)
			db.execute(sqlQry2)
			db.commit()
			response  = Response(status=201)
